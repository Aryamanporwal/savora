...
export const addToCart = TryCatch(async(req: AuthenticatedRequest , res) =>{
    ...
    const cartFromDifferentRetaurant = await Cart.findOne({
        userId , 
        restaurantId : {$ne  : restaurantId},
    }
    );

    if(cartFromDifferentRetaurant){
        return res.status(400).json({
            message : "You can order from only one restaurant at a time. Please clear your cart first to add item from this restaurant.",
        });
    }
...
export const decreamentCrtItem = TryCatch(async(req : AuthenticatedRequest , res) => {
    const userId = req.user?._id
